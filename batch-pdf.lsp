...
(defun BPDF-write-dcl ( / f)
  ...
  (write-line "    : text { label = \"  BPDF  Batch PDF Export  v3.0\"; }" f)
  (write-line "    : text { label = \"================================================\"; }" f)
...
(defun c:BPDF ( / blockname outpath ss i ent obj sel entdata plotMode
  ...
  (cond
    ...
    ((= dResult 2)
     ...
     ;; Use cfg defaults if variables not yet set
     (if (not prefix)   (setq prefix   (cdr (assoc "prefix"   cfg))))
     (if (not outpath)  (setq outpath  (cdr (assoc "outpath"  cfg))))
     (if (not scaleStr) (setq scaleStr (cdr (assoc "scaleStr" cfg))))
     (if (not styleNum) (setq styleNum (atoi (cdr (assoc "styleNum" cfg)))))
     (if (not paperNum) (setq paperNum (atoi (cdr (assoc "paperNum" cfg)))))
     (setq dcl_id (load_dialog (BPDF-dclpath)))
     (if (not (new_dialog "bpdf_dialog" dcl_id))
       (progn (alert "Cannot load dialog.") (exit))
     )
     ...
     (set_tile "outpath"   (if outpath outpath (strcat (getenv "USERPROFILE") "\Desktop")))
